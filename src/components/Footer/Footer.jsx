...
const Footer = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert("thanks for subscribing! we'll send updates ");
    setEmail("");
  };
            ...
            <form onSubmit={handleSubmit} className="relative mt-4 group">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  type="submit"
                >
                  <span className="font-bold text-white tracking-wide transition-transform duration-300 group-hover:-translate-x-1">
                    Join Now
                  </span>
              ...
              <ul className="space-y-3">
                {navItems.map((item) => (
                  <li key={item.name}>
                    <Link
                      to={item.link}
                      className="flex items-center text-amber-400 transition-all group font-lora hover:pl-2"
                    >
                      <BiChevronRight className="mr-2 group-hover:animate-bounce text-amber-400" />
                ...
                {socialIcons.map(
                  ({ icon: Icon, link, color, label }, index) => {
                    return (
                      <Link
                        to={link}
                        className="text-2xl bg-amber-400/10 p-3 rounded-full hover:bg-amber-400/30 hover:scale-105 transition-all duration-300"
                        style={{ color }}
                        key={index}
                        target="_blank"
                      >
                        <Icon className="hover:scale-105 transition-transform" />
